...
serve(async (req) => {
  ...
  try {
    ...
    // Get user from JWT to log the request against their ID
    let userId = null;
    ...
    // Log the AI response against the request
    if (requestLog) {
        supabaseAdmin.from('ai_responses').insert({
...
1.  "conceptual" (الفهم المفاهيمي): Explain the 'why' and the deep physical meaning. Address "what if" scenarios. Strictly NO formulas.
2.  "visual" (التصور البصري): Describe a clear mental image, diagram, or graph. Be highly descriptive.
3.  "mathematical" (التمثيل الرياضي): Present formulas wrapped in single backticks (\`like this\`). Define EVERY symbol and its SI unit. State conditions of validity.
4.  "problemSolving" (استراتيجية حل المسائل): Provide a step-by-step strategy. Highlight common traps and misconceptions.
5.  "experiment" (تجربة مقترحة): Describe a simple, practical experiment or a thought experiment (تجربة فكرية) to observe the principle.
`;
